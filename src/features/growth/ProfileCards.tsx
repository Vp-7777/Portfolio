...
const profiles = [
  ...
  },
  {
    ...
    id: "Vp-7777",
    url: "https://leetcode.com/Vp-7777",
...
export function ProfileCards() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 pt-8 border-t border-border-subtle/40 select-none">
      {profiles.map((p) => (
        <a
          key={p.name}
          href={p.url}
          ...
          onMouseEnter={() => handleMouseEnter(p.name)}
          onMouseLeave={handleMouseLeave}
          className="spotlight-card group p-6 rounded-xl space-y-6 flex flex-col justify-between hover:border-purple-500/50 transition-all duration-300 cursor-pointer block"
        ...
        >
          ...
          <div className="flex justify-between items-start relative z-base">
            <div className="space-y-1">
              ...
              <span className="font-sans text-xs text-muted/80 tracking-wide font-medium block">
                {p.label}
              </span>
            </div>
            {p.svg}
          ...
          <div className="flex justify-between items-end pt-4 border-t border-border-subtle/40 relative z-base font-sans text-xs text-muted font-medium">
            <span>@{p.id}</span>
            <span className="text-cyan-400 font-semibold">{p.metric}</span>
          </div>
