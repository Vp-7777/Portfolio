...
export function Hero() {
  ...
  return (
    ...
    >
      {/* Luxury Ambient Editorial Lighting (Soft silk radial warmth, no distracting wireframes) */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 z-0 select-none overflow-hidden"
      >
        <div className="absolute top-[10%] left-1/2 -translate-x-1/2 w-[900px] h-[550px] rounded-full bg-gradient-to-b from-[#EEE6D4]/80 via-[#EEE6D4]/30 to-transparent blur-[120px] opacity-70" />
        <div className="absolute top-28 left-8 text-[#A9793C]/30 font-mono text-xs select-none hidden lg:block">
          +
        </div>
        <div className="absolute top-28 right-8 text-[#A9793C]/30 font-mono text-xs select-none hidden lg:block">
          +
        </div>
      </div>

      <div className="max-w-[1180px] mx-auto px-6 sm:px-8 w-full space-y-12 sm:space-y-16 relative z-10">
        ...
        <div className="flex items-center gap-3 font-mono text-xs sm:text-[13px] text-[#7C5A2C] font-semibold tracking-[0.08em] uppercase select-none">
          ...
          <span>VISHAL PATEL — SOFTWARE & AI ENGINEER — CHENNAI, IN</span>
        </div>

        {/* 02: Oversized Fraunces Thesis Statement (Word-by-Word Staggered Reveal) */}
        <h1 className="font-display font-light text-[2.6rem] sm:text-[4rem] md:text-[5.2rem] lg:text-[6.2rem] leading-[1.02] tracking-[-0.03em] text-[#1B1710] max-w-5xl">
        ...
        {/* 04: Asymmetric Meta Row */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-start pb-16 sm:pb-24">
          
          {/* Left Column (7 cols): Supporting Narrative Bio */}
          ...
          <div className="md:col-span-5 flex flex-wrap items-center md:justify-end gap-5 pt-1">
            
            {/* Magnetic Filled Button */}
            <button
              ref={magneticButtonRef}
              onMouseMove={handleMouseMove}
              onMouseLeave={handleMouseLeave}
              onClick={() => scrollToSection("work")}
              className="px-7 py-3.5 bg-[#1B1710] hover:bg-[#7C5A2C] text-[#F6F1E7] font-mono text-xs uppercase tracking-wider transition-colors duration-200 cursor-pointer shadow-xs flex items-center gap-2 group"
              style={{
